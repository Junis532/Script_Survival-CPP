            ...
            if (!inventory[current_rarity][i].is_was_having) {
                ...
                if (current_rarity == RARITY_UNIQUE) {
                    ...
                    if (player->weapon_index == player->armor_index &&
                        ...
                        utils.utils_set_color(COLOR_YELLOW);
                    }
                    else {
...
void Dynamic::UI_dynamic_player_name_input(void)
{
    ...
    for (int i = 0; i < box_height; i++) {
        utils.utils_gotoxy(start_x, start_y + i); putchar('|');
        utils.utils_gotoxy(start_x + box_width - 1, start_y + i); putchar('|');
    }

    for (int i = 0; i < box_width; i++) {
        utils.utils_gotoxy(start_x + i, start_y); putchar('=');
        utils.utils_gotoxy(start_x + i, start_y + box_height - 1); putchar('=');
    }
...
void Dynamic::UI_dynamic_title_selection(int ui_title_state)
{
    const char* menu[] = {
       "시작",
       "옵션",
...
void Dynamic::UI_dynamic_setting_menu(setting_state_t selected, int* global_volume)
{
    ...
    utils.utils_gotoxy(mx_vol, vol_y);
...
void Dynamic::UI_dynamin_esc_menu(esc_menu_state_t* selected, int gamemode)
{
...
void Dynamic::UI_dynamic_save_load_menu(save_load_num_t* selected)
{
    int box_width = 38;
...
void Dynamic::UI_dynamic_select_game_mode(game_mode_state_t selected, bool is_infinite_unlocked)
{
    const char* modes[] = {
       "일반 모드",
       "무한 모드"
    ...
    for (int i = 0; i < 2; i++) {
        ...
        utils.utils_gotoxy(mx, 13 + i * 2);

        if (i == MODE_STATE_INFINITY && !is_infinite_unlocked) {
            utils.utils_set_color(COLOR_DARKGRAY); // 잠겼으면 회색으로
        }
        else if (selected == i) {
            utils.utils_set_color(COLOR_SELECT_MENU);
        }
...
void Dynamic::UI_dynamic_select_new_or_load_game(new_or_load_game_t* selected)
{
    const char* modes[] = {
       "새 게임",
       "불러오기"
...
void Dynamic::UI_dynamic_hero_select(hero_t selected_hero)
{
    int box_width = 38;
    int padding = (WIDTH - (box_width * 3)) / 4;
...
void Dynamic::UI_dynamic_infinite_upgrade(player_t* player, const upgrade_type_t choices[], int selection)
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (i == selection) {
            utils.utils_set_color(COLOR_SELECT_MENU); // 선택된 항목은 흰색
        }
        else {
            utils.utils_set_color(COLOR_DEFAULT);     // 선택되지 않은 항목은 회색
        }
...
// 배틀 선택지 ui 그리는 함수
void Dynamic::UI_dynamic_player_action_selection(int player_action_state)
{
    const char* menu[] = {
       "Attack",
       "Skill",
...
static void s_UI_dynamic_monster_info_internal(monster_t* monster, int body_color)
{
    ...
    utils.utils_set_color(body_color);
    ...
    for (int i = 0; i < lines; i++) {
        ...
        utils.utils_gotoxy(start_x, start_y + i);
...
void Dynamic::UI_dynamic_monster_info(monster_t* monster, int current_stage)
{
    s_UI_dynamic_monster_info_internal(monster, COLOR_DEFAULT_TEXT);

    utils.utils_gotoxy(50, 19);
    utils.utils_set_color(COLOR_LIGHTGRAY);
    printf("스테이지 : ");
    printf("%d", current_stage + 1);
    utils.utils_gotoxy(80, 19);
    printf("몬스터 : ");
...
void Dynamic::UI_dynamic_player_info(player_t* player)
{
    utils.utils_gotoxy(114, 21);  printf("Name : %s", player->name);
    utils.utils_gotoxy(114, 22);  printf("HP   : %5d / %5d", get_player_hp(player), get_player_max_hp(player));
    utils.utils_gotoxy(114, 23);  printf("ATK  : %d", get_player_attack(player));
    utils.utils_gotoxy(114, 24);  printf("SPD  : %d", get_player_speed(player));
    utils.utils_gotoxy(114, 25);  printf("EVA  : %.2f%%", get_player_evasion_rate(player) * 100);
    utils.utils_gotoxy(114, 26);  printf("DEF  : %.2f%%", get_player_defense_rate(player) * 100);
}

void Dynamic::UI_dynamic_select_heal_or_store(heal_or_store_t* selected, player_t* player) {
...
void Dynamic::UI_dynamic_action_order(player_t* player, monster_t* monster, int field_action_value, int field_speed)
{
    // 임시 변수에 현재 행동 가치를 복사
    ...
    for (int i = 0; i < 5; i++) {
        ...
        if (p_av <= m_av && p_av <= f_av) {
            ...
            if (monster_turn % 3 == 0) utils.utils_set_color(COLOR_RED);
            else utils.utils_set_color(COLOR_DEFAULT_TEXT);
            monster_turn++;
            printf("[ 몬스터 ]");
            m_av += m_base_av;
            utils.utils_set_color(COLOR_DEFAULT_TEXT);
...
// [수정] 
void Dynamic::UI_dynamic_inventory_info(player_t* player)
{
    ...
    for (int i = 0; i < 5; i++) {
        ...
        utils.utils_gotoxy(top_items[i].x, top_items[i].y);
...
// [수정] 
void Dynamic::UI_dynamic_store_info(player_t* player)
{
    ...
    for (int i = 0; i < 4; i++) {
        ...
        utils.utils_gotoxy(store_menu[i].x, store_menu[i].y);
...
void Dynamic::UI_dynamic_current_weapon_info(player_t* player)
{
    int start_x = 0;
    int end_x = 38;
    if (player->weapon_index == -1) {
...
void Dynamic::UI_dynamic_current_armor_info(player_t* player)
{
    int start_x = 38;
    int end_x = 75;
