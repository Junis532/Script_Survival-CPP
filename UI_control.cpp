...
void UI_control_setting(UI_state_t* ui_main_state, setting_state_t* ui_setting_state, bool is_come_esc_menu, int* global_volume, int key)
{
    ...
    case LEFT:
        if (*ui_setting_state == SETTING_STATE_VOLUME && *global_volume > 0)
            *global_volume -= 5;
        utils.utils_set_volume_from_percentage(*global_volume);
        break;
    ...
    case ENTER:
        if (*ui_setting_state == SETTING_STATE_SOUND_ON) {
            utils.utils_sound_set_enabled(true);
            utils.utils_sound_play(TEXT("SFX/SoundEffect/sound_on.wav"));
        }
        else if (*ui_setting_state == SETTING_STATE_SOUND_OFF) {
            utils.utils_sound_play(TEXT("SFX/SoundEffect/sound_off.wav"));
            Sleep(500);
            utils.utils_sound_set_enabled(false);
        }
    ...
    else if (focus_level == FOCUS_LEVEL_ITEM_BUY_SELL)
    {
        if (menu_key == ESC) { focus_level = FOCUS_LEVEL_ITEM_LIST; }
            ...
            if (buy_sell_state == STORE_STATE_BUY) {
                utils.utils_sound_play(TEXT("SFX/SoundEffect/store_buy.wav"));
