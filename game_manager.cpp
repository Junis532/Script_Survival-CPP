...
void GameManager::Init() {
    ...
    g_context.is_normal_mode_cleared = load_clear_status();
    g_context.is_change_ui_main = true;
    g_context.global_volume = 50;
    g_context.upgrade_selection = 0;
    g_context.ui_esc_menu_state = ESC_MENU_STATE_BACK;
    g_context.save_load_num = SAVE_LOAD_1;
    g_context.new_or_load_game = NEW_GAME;
    g_context.heal_or_store_state = HEAL_OR_STORE_HEAL;
    g_context.field_action_value = 0;
    g_context.field_speed = 300;
        ...
        if (g_context.is_change_ui_main) {
            ...
            if (g_context.ui_main_state == UI_STATE_TITLE) {
                if (g_context.is_normal_mode_cleared)
                    uiStatic.UI_static_hide_title();
                else
                    uiStatic.UI_static_title();
            }
            else { // UI_STATE_SETTING
                uiStatic.UI_static_setting_menu();
            }
...
static void state_battle(bool* is_game_over) {
    ...
    if (g_context.player.action_value <= g_context.monster.action_value
        ...
        dynamic.UI_dynamic_player_action_selection(g_context.player_action_state);
        int key = utils.utils_getch();
