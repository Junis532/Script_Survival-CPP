...
static void s_log_print_buffer(void)
{
    ...
    for (int i = 0; i < LOG_MAX_LINES; i++) {
       utils. utils_gotoxy(LOG_START_X, LOG_START_Y + i);
        if (s_log_buffer[i].text) {
            if (i == current_index) {
                ...
                if (s_log_buffer[i].is_critical) {
                    ...
                    utils.utils_set_color(COLOR_CURRENT_LOG);
                }
                ...
                // 이전 로그들은 회색으로 표시
                if (s_log_buffer[i].is_critical) {
                    utils.utils_set_color(COLOR_BROWN); // 이전 치명타 로그는 갈색으로
                }
                else {
                    utils.utils_set_color(COLOR_HISTORY_LOG);
                }
