...
void Field::field_effect_on(player_t* player, monster_t* monster, int type)
{
	...
	if (type == 0) { // 행동서열 바꾸기
		utils.utils_swap_double(&player->action_value, &monster->action_value);
		logg.log_field_effect_swap_action_value();
	}
		...
		logg.log_field_effect_player_damage(player, player_damage);
		logg.log_field_effect_monster_damage(monster, monster_damage);
	}
		...
		logg.log_field_effect_zero_evasion();
	}
		...
		logg.log_field_effect_attack_increase();
	}
	else if (type == 4) { // 광역힐
		...
		if(player->action_value >= monster->action_value) {
			...
			logg.log_field_effect_blood_for_power(player->name, sum_attack);
		}
		else {
			monster->attack += sum_attack; // 몬스터 공격력 증가
			is_add_attack_player = false; // 몬스터가 공격력 증가
			logg.log_field_effect_blood_for_power(monster->name, sum_attack);
		}
	}
		...
		if (player_per <= monster_per)
		{
			...
			logg.log_field_effect_power_attack(monster->name);
		}
		else
		{
			set_player_hp(player, get_player_hp(player) / 5); // 플레이어에게 강력한 피해
			logg.log_field_effect_power_attack(player->name);
		}
	}
}
